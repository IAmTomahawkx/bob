...
class Idevision(commands.Cog):
    ...
    async def add(self, ctx: "Context", quick=None, long=None, url=None):
        ...
        try:
            long = await commands.clean_content().convert(ctx, (await ctx.ask("Now, please provide the full documentation name", return_bool=False)).content)
            check_cancel(long)
        ...
        async with self.bot.session.get(url.strip("/")+"/objects.inv") as resp:
            if resp.status != 200:
                raise commands.CommandError("Invalid url provided (no /objects.inv found). remember to remove the current page! ex. https://docs.readthedocs.io/latest")
    ...
    async def chess(self, ctx: "Context", target: discord.User, board: str="walnut"):
        ...
        th2 = (await ctx.ask(f"{target.mention}, theme?", return_bool=False, predicate=lambda m: m.author == target and m.channel == ctx.channel)).content

        async with self.session.post(self.url + "games/chess", json={"white-theme": th1, "black-theme": th2, "board-theme": board}) as resp:
        ...
        while True:
            ...
            while True:
                move = (await ctx.ask(None, return_bool=False, predicate=lambda m: m.author == t and m.channel == ctx.channel)).content
            ...
            async with self.session.post(self.url + "games/chess/turn", json={"board": board, "move": move, "move-turn": bool(turn)}) as resp:
                if resp.status == 417:
                    data = await resp.json()
                    board = data['board']
    ...
    async def rtfs(self, ctx: "Context", lib: Optional[_libconverter], *, item):
        ...
        if fmt == "links":
            ...
            await ctx.send(embed=ctx.embed_invis(description="\n".join(out), url="https://idevision.net/docs",
                                                 title="API result:").set_footer(text=f"query time: {round(time, 3)}"))
        else:
    ...
    async def ocr(self, ctx, attachment: str=None):
        ...
        filetype = (ctx.message.attachments[0].filename if ctx.message.attachments else attachment.split("/")[-1]).split(".")[-1]
    ...
    async def api_apply(self, ctx: "Context", *, reason: str):
        ...
        async with self.session.post(self.url + "internal/users/apply", json={
            ...
            if resp.status == 403:
                ...
                emb = emb.set_author(name=str(ctx.author), icon_url=str(ctx.author.avatar_url)).set_footer(text=str(ctx.author.id))
    ...
    async def cdn_upload(self, ctx: "Context", *args):
        ...
        async with ctx.typing():
            ...
            if url:
                fn = url.split("/")[-1]
                async with self.bot.session.get(url, headers={"Accept": "image/*"}) as resp: # type: aiohttp.ClientResponse
    ...
    async def addroute(self, ctx, route: str, method: str, perm: str=None):
        if all((route, method, perm)):
            async with self.session.post(self.url + "internal/routes", json={
                "endpoint": route,
                "method": method,
                "permission": perm
            }) as resp:
    ...
    async def toggle_beta(self, ctx, state: bool=None):
        if state is None:
            return await ctx.reply(f"Currently targeting {'beta' if self.url == 'https://beta.idevision.net/api/' else 'production'} idevision site ({self.url.replace('api/', '')})")
        ...
        await ctx.reply(f"Now targeting {'beta' if self.url == 'https://beta.idevision.net/api/' else 'production'} idevision site ({self.url.replace('api/', '')})")
